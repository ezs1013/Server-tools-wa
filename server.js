...
app.post('/api/redeem/create', function(req, res) {
    var ownerKey=req.body.owner_key;
    ...
    for (var i=0;i<count;i++) {
        ...
        redeemCodes[code]={features:features,used:false,createdAt:dateStr};
        generated.push({code:code,features:features});
    }
    saveCodes();
...
app.get('/api/redeem/list', function(req, res) {
    ...
    res.json({success:true,codes:redeemCodes});
});
...
app.post('/api/redeem/use', function(req, res) {
    ...
    var entry=redeemCodes[inputCode];
    ...
    delete redeemCodes[inputCode];
    saveCodes();
    res.json({valid:true,features:features,message:'Kode berhasil digunakan!'});
});
